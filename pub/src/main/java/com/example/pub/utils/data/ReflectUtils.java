...
public class ReflectUtils {
    ...
    * ******************************************************* */
     ...
     * @return
     */
    public static Field getField(Class<?> sourceClass, String fieldName, boolean isFindDeclaredField, boolean
            isUpwardFind) {
        Field field = null;
        try {
        ...
        if (isGetParentField) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Field field : isGetDeclaredField ? classList.get(w).getDeclaredFields() : classList.get(w)
                            .getFields()) {
                        fieldList.add(field);
                    }
                }
     ...
     */
    public static boolean setField(Object object, String fieldName, Object newValue, boolean isFindDeclaredField,
    ...
    * ******************************************************* */
     ...
     */
    public static Method getMethod(Class<?> sourceClass, boolean isFindDeclaredMethod, boolean isUpwardFind, String
            methodName, Class<?>... methodParameterTypes) {
        Method method = null;
        try {
            ...
            if (isUpwardFind) {
                ...
                while (method == null && classs != null) {
                    try {
                        method = isFindDeclaredMethod ? classs.getDeclaredMethod(methodName, methodParameterTypes) :
     ...
     * @return 给定类的所有方法
     */
    public static List<Method> getMethods(Class<?> clas, boolean isGetDeclaredMethod, boolean isFromSuperClassGet,
                                          boolean isDESCGet) {
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Method method : isGetDeclaredMethod ? classList.get(w).getDeclaredMethods() : classList.get
                            (w).getMethods()) {
    ...
    * ******************************************************* */
     ...
     */
    public static Constructor<?> getConstructor(Class<?> sourceClass, boolean isFindDeclaredConstructor, boolean
            isUpwardFind, Class<?>... constructorParameterTypes) {
        ...
        try {
            ...
            if (isUpwardFind) {
                ...
                while (method == null && classs != null) {
                    try {
                        method = isFindDeclaredConstructor ? sourceClass.getDeclaredConstructor
                                (constructorParameterTypes) : sourceClass.getConstructor(constructorParameterTypes);
                    } catch (NoSuchMethodException e11) {
                        classs = classs.getSuperclass();
                    }
                }
     ...
     * @return 给定的类中所有的构造函数
     */
    public static List<Constructor<?>> getConstructors(Class<?> sourceClass, boolean isFindDeclaredConstructor,
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Constructor<?> constructor : isFindDeclaredConstructor ? classList.get(w)
                            .getDeclaredConstructors() : classList.get(w).getConstructors()) {
            ...
            for (Constructor<?> constructor : isFindDeclaredConstructor ? sourceClass.getDeclaredConstructors() :
	...
	* ******************************************************* */
    ...
    public static <T> T getObjectByFieldName(Object object, String fieldName, Class<T> clas) {
        if (object != null && StringUtils.isEmpty(fieldName) && clas != null) {
            try {
    ...
    public static final boolean isCollectionByType(Field field, Class<? extends Collection> collectionType, Class<?>
            type) {
        Class<?> fieldType = field.getType();
