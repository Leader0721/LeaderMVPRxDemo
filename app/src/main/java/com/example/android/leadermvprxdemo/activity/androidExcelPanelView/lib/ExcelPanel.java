package com.example.android.leadermvprxdemo.activity.androidExcelPanelView.lib;
...
public class ExcelPanel extends FrameLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public ExcelPanel(Context context, AttributeSet attrs) {
        ...
        TypedArray a = getContext().getTheme().obtainStyledAttributes(
                ...
                0, 0);
        try {
            leftCellWidth = (int) a.getDimension(R.styleable.ExcelPanel_left_cell_width, DensityUtils.dp2px(DEFAULT_LENGTH));
            topCellHeight = (int) a.getDimension(R.styleable.ExcelPanel_top_cell_height, DensityUtils.dp2px(DEFAULT_LENGTH));
            normalCellWidth = (int) a.getDimension(R.styleable.ExcelPanel_normal_cell_width, DensityUtils.dp2px(DEFAULT_LENGTH));
        } finally {
            a.recycle();
        }
        indexHeight = new TreeMap<>();
        indexWidth = new TreeMap<>();
        loadingViewWidth = DensityUtils.dp2px(LOADING_VIEW_WIDTH);
        initWidget();
    }
    ...
    protected View createDividerToLeftHeader() {
        ...
        view.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.tvc9));
        return view;
    }
