package com.example.android.leadermvprxdemo.activity.weatherView;
...
public class FutureDaysChart extends View {
    ...
    private void init(Context context){
        pointRaidus=TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,3f,getResources().getDisplayMetrics());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < lineSize; i++) {
            ...
            float y=chartHeight/2f-padding/2-(datas.get(i).getHighTemperature()-minHigh)*perHeightTop;
            ...
            if(isCubic){
                if (Float.isNaN(currentPointX)) {
                    ...
                    currentPointY = chartHeight/2f-padding/2-(datas.get(i).getHighTemperature()-minHigh)*perHeightTop;
                }
                if (Float.isNaN(previousPointX)) {
                    if (i > 0) {
                        previousPointX = eachWidth/2f+(i-1)*eachWidth;
                        previousPointY = chartHeight/2f-padding/2-(datas.get(i-1).getHighTemperature()-minHigh)*perHeightTop;
                    } else {
                ...
                if (Float.isNaN(prePreviousPointX)) {
                    if (i > 1) {
                        ...
                        prePreviousPointY = chartHeight/2f-padding/2-(datas.get(i-2).getHighTemperature()-minHigh)*perHeightTop;
                    } else {
                ...
                if (i < lineSize - 1) {
                    ...
                    nextPointY = chartHeight/2f-padding/2-(datas.get(i+1).getHighTemperature()-minHigh)*perHeightTop;
                } else {
            ...
            canvas.drawText(label,x-labelPaint.measureText(label)/2,y-pointRaidus*4,labelPaint);
        }
        ...
        for (int i = 0; i < lineSize; i++) {
            float x=eachWidth/2f+i*eachWidth;
            float y=chartHeight-padding/2-(datas.get(i).getLowTemperature()-minLow)*perHeightBottom;
            canvas.drawCircle(x,y,pointRaidus,pointPaint);
            ...
            if(isCubic){
                if (Float.isNaN(currentPointX)) {
                    currentPointX = eachWidth/2f+i*eachWidth;
                    currentPointY = chartHeight-padding/2-(datas.get(i).getLowTemperature()-minLow)*perHeightBottom;
                }
                if (Float.isNaN(previousPointX)) {
                    if (i > 0) {
                        previousPointX = eachWidth/2f+(i-1)*eachWidth;
                        previousPointY = chartHeight-padding/2-(datas.get(i-1).getLowTemperature()-minLow)*perHeightBottom;
                    } else {
                        previousPointX = currentPointX;
                        previousPointY = currentPointY;
                    }
                ...
                if (Float.isNaN(prePreviousPointX)) {
                    if (i > 1) {
                        prePreviousPointX = eachWidth/2f+(i-2)*eachWidth;
                        prePreviousPointY = chartHeight-padding/2-(datas.get(i-2).getLowTemperature()-minLow)*perHeightBottom;
                    } else {
                        prePreviousPointX = previousPointX;
                        prePreviousPointY = previousPointY;
                    }
                ...
                if (i < lineSize - 1) {
                    nextPointX = eachWidth/2f+(i+1)*eachWidth;
                    nextPointY = chartHeight-padding/2-(datas.get(i+1).getLowTemperature()-minLow)*perHeightBottom;
                } else {
                    nextPointX = currentPointX;
                    nextPointY = currentPointY;
                }
